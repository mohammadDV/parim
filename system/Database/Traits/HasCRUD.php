...
trait HasCRUD {
    ...
    protected function saveMethod(){
        ...
        if (!isset($this->{$this->primaryKey})){
            $query = "INSERT INTO " . $this->getTableName() . " SET " . $fillString;
            if (!empty($this->createdAt)){
                $query .= " , " . $this->getAttrName($this->createdAt) . " = " . time();
            }

            $this->setSql($query);
        }else{
            $query = "UPDATE  " . $this->getTableName() . " SET " . $fillString;
            if (!empty($this->updatesAt)){
                $query .= " , " . $this->getAttrName($this->updatesAt) . " = " . time();
            }
            $this->setSql($query);
            $this->setWhere("AND",$this->getAttrName($this->primaryKey) . " = ? ");
        ...
        if (!isset($this->{$this->primaryKey}) && $this->primaryKey !== false){
